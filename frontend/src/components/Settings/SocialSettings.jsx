...
export default function SocialSettings() {
  ...
  const [addButtonChange, setAddButtonChange] = useState(null)
  ...
  const handleAddContact = (e) => {
    ...
    setAddButtonChange(!addButtonChange)
  ...
  const contactButton = () => {
    ...
    if (receivedRequests.some(request => request.sender === contactFound.id)
      || pendingRequests.some(request => request.recipient === contactFound.id)) {
      return <p>Pending</p>
    } if (contactFound.username) {
      return <button onClick={handleAddContact}>Add contact</button>
    }
  }
  ...
  return (
    <>
      ...
      {showReceived
        ...
        : <PendingRequests pendingRequests={pendingRequests} setTriggerFetch={setTriggerFetch} />
      }
      <h3>Contacts</h3>
      {
